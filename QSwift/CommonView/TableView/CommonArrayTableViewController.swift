...
//            }
            ...
            self .readDataFromLocal(isRefresh: isRefresh)
        })
    }
    
    func readDataFromLocal(isRefresh: Bool! = true){
        /// 子线程读取本地数据 主线程刷新
//        DispatchQueue.global().async {
            //BaseResponse<BasePagerModel<UserSimpleModel>>
        var userlistPlist:String = ""
        if(arrayApiPath() == "user/getlist"){
            userlistPlist = Bundle.main.path(forResource: "userlistsmall", ofType: "plist")!
        }else{
            userlistPlist = Bundle.main.path(forResource: "topiclists", ofType: "plist")!
        }
     
        let userlistDictPlist = NSDictionary(contentsOfFile: userlistPlist)
//        let tempBaseResponseModel = JSONDeserializer<BaseResponse<BasePagerModel<UserSimpleModel>>>.deserializeFrom(dict: userlistDictPlist, designatedPath: "") as! BaseResponse
        let tempBaseResponseModel = JSONDeserializer<BaseResponse<BasePagerModel<T>>>.deserializeFrom(dict: userlistDictPlist)
        
        if(isRefresh){
            self.endHeaderRefreshing()
            self.hideCenterLoadingView()
            if(tempBaseResponseModel!.code == 1){
                self.mainArray.removeAll()
                //mainArray
                //  self.mainArray.append(contentsOf: (response.data?.items)!)
                //  self.mainArray.append(contentsOf: (response.data?.items)!)
//                mainArray.append(tempBaseResponseModel.data?.items)
                self.mainArray.append(contentsOf: (tempBaseResponseModel!.data?.items)!)

//                self.mainArray.append(contentsOf: (tempBaseResponseModel.data?.items)!)
                self.hideRefreshFooter(hide: !tempBaseResponseModel!.data!.hasMore())
                self.tableView.reloadData()
                self.needShowEmptyView(show: self.mainArray.isEmpty, emptyImage: nil, emptyTitle: nil)
                self.page = 2;
                
            }else{
                printLog(tempBaseResponseModel!.message)
                self.needShowEmptyView(show: self.mainArray.isEmpty, emptyImage: nil, emptyTitle: tempBaseResponseModel!.message)
            }
        }else{
            self.endFooterRefreshing()
            if(tempBaseResponseModel!.isSuccess()){
                self.mainArray.append(contentsOf: (tempBaseResponseModel!.data?.items)!)
                self.hideRefreshFooter(hide: !tempBaseResponseModel!.data!.hasMore())
                self.tableView .reloadData()
                self.page += 1
